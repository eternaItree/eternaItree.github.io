...
So today, we're going to learn how to take an arbitrary binary only target that takes an input file from the user and turn it into a target that takes it's input from memory instead and lends itself well to having its state reset between fuzzcases. 
